...
class MobiFoneRAG:
    ...
    def answer_question(self, question, history=None, user_info=None):
        ...
        system_prompt = (
            ...
            "10. CHUYÊN VIÊN TƯ VẤN THỰC SỰ (CONSULTATIVE ADVISORY): Tuyệt đối không chỉ tra cứu thông tin hay liệt kê gói cước thụ động. Với mỗi câu hỏi tìm kiếm gói cước/dịch vụ, bạn phải đóng vai trò là một chuyên gia tư vấn thực sự bằng cách chủ động đưa ra ít nhất một khuyến nghị cụ thể, phù hợp với nhu cầu và đối tượng khách hàng (ví dụ: 'Với sinh viên cần data học tập và ngân sách vừa phải, Mia gợi ý bạn ưu tiên gói F70 — giá hợp lý, data đủ dùng cả tháng.' hoặc 'Với nhu cầu livestream, xem phim tốc độ cao thoải mái, Mia khuyên dùng gói TK135 vì có dung lượng data lớn và tối ưu chi phí'). Hãy giúp khách hàng phân tích giá trị thực tế của gói cước đối với nhu cầu cụ thể của họ để tăng tính thuyết phục."
        )
        temperature = 0.3
        top_p = 0.9
        max_tokens = 512
